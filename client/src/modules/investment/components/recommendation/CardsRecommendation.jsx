...
export const CardsRecommendation = () => {
  ...
  const onHandleRecommendations = async () => {
    try {
      ...
      // Generar datos ficticios basados en los símbolos
      const mappedRecommendations = newRecommendations.map((symbol) => ({
        type: symbol === "BND" ? "Conservador" : "Moderado",
        title: symbol,
        details: [
          { label: "Rendimiento:", value: symbol === "BND" ? "4.5%" : "5.3%" },
          { label: "Riesgo:", value: symbol === "BND" ? "Bajo" : "Medio" },
        ],
        instrument: symbol,
        id: `${symbol}`, // Genera un ID único
      }));
